...
func vX_20TaskExecutor_10Arm (clap <-chan []string, flap chan<- []string) {
	...
	for {
		...
		//_|^^2^^|
		k1100, _ := strconv.Atoi (gjson.Get (_x1900 [2],
			"description.period").String ())
		if k1100 < 0 || k1100 > 4838400 {
			_k2200 := make (map[string]interface {})
			_k2200 ["id"] = gjson.Get (_x1900 [2], "id").String ()
			_k2300 := make (map[string]interface {})
			_k2300 ["success_status"] = "4"
			_k2200 ["description"] = _k2300			
			_k2400, _ := json.Marshal (_k2200)
			flap <- []string {"20", "30", string (_k2400)}
			continue
		}
		//_|==2==|
		...
		//_|^^2^^|
		_m2450 := strconv.Itoa (int (time.Now ().Unix () + int64 (k1100)))
		_, _m2610 := _f1400.Exec ("update x1200____session set " +
			"expiry_time = $1 where __user = $2 and id = $3 and key = $4", _m2450,
			gjson.Get (_x1900 [2], "description.user_id").String (),
			gjson.Get (_x1900 [2], "description.session.id").String (),
			gjson.Get (_x1900 [2], "description.session.key").String (),
		)
		if _m2610 != nil {
			_m2700 := fmt.Sprintf ("Reporter: Component 20 (Task " +
				"Executor)\n" +
				"Time: %s\n" +
				"Request Id: %s\n" +
				"Request Description: %s\n" +
				...
				gjson.Get (_x1900 [2], "description").String (),
				_m2610.Error ())
			flap <- []string {"20", "10", _m2700}
			
			_m2800 := make (map[string]interface {})
			_m2800 ["id"] = gjson.Get (_x1900 [2], "id").String ()
			_m2900 := make (map[string]interface {})
			_m2900 ["success_status"] = "3"
			_m2800 ["description"] = _m2900			
			_m3000, _ := json.Marshal (_m2800)
			flap <- []string {"20", "30", string (_m3000)}
			continue
		}
		//_|==2==|
